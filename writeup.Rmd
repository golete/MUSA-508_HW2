...
+ C. Census data from the 2019 ACS, including home ownership and vacancy rates, education, income and demographics on the block group level.
+ D. Other open data, including FEMA, municipal services or amenities and crime data, if pertinent.
+ E. “Experimental” data, meaning variables like location of recreational cannabis dispensaries or distance to major highways, that are not conventionally associated with house prices.
...
highwayData <- homeIDs %>%
  mutate(highwayDistance = log(drop_units(st_distance(., coloradoHighways)))) %>%
  st_drop_geometry()
...
errorTable <- data.frame(MAE = mean(homes.test$price.AbsError),
                         MAPE = mean(homes.test$price.APE))

# generate polished table
errorTable %>%
  kbl(
    caption = "Mean average and mean absolute percent errors",
    digits = 3,
    format.args = list(big.mark = ",")
  ) %>%
...
# TODO: map residuals for test set
...
st_join(bothRegressions, incomeTest) %>% 
  ...
  kable(., caption = "Test set MAPE by household income context") %>% kable_classic()

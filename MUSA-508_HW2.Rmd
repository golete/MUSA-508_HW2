...
tracts <- 
  ...
  mutate(PCTHHowner = HHownerOc/HHtotal,
         ...
         PCTbel125pov = (B17026_002E+B17026_003E+B17026_004E+B17026_005E)/B17026_001E,
         PCT125185pov = (B17026_006E+B17026_007E+B17026_008E)/B17026_001E,
         PCT185500pov = (B17026_009E+B17026_010E+B17026_011E+B17026_012E)/B17026_001E,
         PCTabo500pov = 1-PCTbel125pov - PCT125185pov - PCT185500pov) %>%
  select(-HHtotal, -HUtotal, -HHvacant, -HHownerOc,-HHwhite,
         -starts_with('Edu'), -starts_with('B17026')) %>%
  st_transform(boulderCRS) 
...
```{r Model estimation, echo= F, warning=F, error=F, message=F}

# 4. MODEL ESTIMATION & VALIDATION

# Filter out outliers and seeming err

dataset <- dataset %>% filter(!MUSA_ID %in% c(8735,1397))
...
inTrain <- createDataPartition(
  y = paste(
    regData$extWall,
    regData$floodRisk,
    regData$neighborhood   #take out neighborhoods to test Neighborhood effect
  ),
  p = 0.75, list = FALSE)
...
homes.test <- homes.test %>%
  mutate(
    ...
    price.Predict = exp(predict(reg.training, homes.test)),
    price.Error = price.Predict - price,
    price.AbsError = abs(price.Predict - price),
    price.APE = (abs(price.Predict - price)/price.Predict)    
  )

mean(homes.test$price.AbsError, na.rm = T) # MAE
mean(homes.test$price.APE, na.rm = T)      # MAPE

# Plot distribution of prediction errors

hist(homes.test$price.Error, breaks = 50) # Its not plotting or giving a warning???
...
homes.test %>%
  ...
  ggplot(aes(x=lagPriceError, y=price.Error))
...
reg.nhood <- lm(price ~ ., data = as.data.frame(homes.training) %>% 
...
homes.test.nhood <-
  ...
  mutate(Regression = "Neighborhood Effects",
         price.Predict = predict(reg.nhood, homes.test),
         price.Error = price.Predict- price,
         price.AbsError = abs(price.Predict- price),
...
# Produce a table comparing neighborhood effects
...
# Why is the Neighborhood effects APE so bad?
...
# Replace with getting income data from our dataset?
...
povLine <- tracts %>%
  ...
  mutate(percentWealth = notWealth / totalPop,

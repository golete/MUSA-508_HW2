...
tracts <- 
  ...
  mutate(PCTVacant = HHvacant/HUtotal) %>%
  ...
  select(-HHtotal, -HUtotal, -HHvacant, -HHownerOc,-HHwhite,-starts_with('Edu'), -starts_with('B17026')) %>%
  st_transform(boulderCRS) 
...
# Exclude homes over $10 million (or not, to keep)
...
regData <- d2n # Update with tested model
...
# TPlot distribution of prediction errors
...
# THIS PART WAS NOT WORKING WITH MY CODE


reg.nhood <- lm(price ~ ., data = as.data.frame(homes.training) %>% 
                  dplyr::select(neighborhood, price)) # ADD VARS USED BEFORE???
...
homes.test$Regression = "Baseline"


aaa <- dplyr::select(homes.test, starts_with("price"), Regression, neighborhood) %>%
  mutate(lagPriceError = lag.listw(spatialWeights.test, price.Error))

bbb <- dplyr::select(homes.test.nhood, starts_with("price"), Regression, neighborhood) %>%
  mutate(lagPriceError = lag.listw(spatialWeights.test, price.Error))¡¡¡¡¡=

dim(aaa)
dim(bbb)

bothRegressions <- rbind(aaa,bbb) 
...
# *******************************************
# below this the code is not translated from the book

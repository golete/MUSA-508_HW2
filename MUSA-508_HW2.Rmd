...
#   rename(floodRisk = FLD_ZONE)
# 
# floodData <- st_join(homeIDs, floodplains2) %>%
#   mutate(floodRisk = replace_na(floodRisk, 0)) %>%
#   st_drop_geometry()
...
st_drop_geometry(finalData) %>% 
  dplyr::select(logPrice, qualityNum) %>%
  # mutate(qualityNum = as.factor(qualityNum)) %>%
...
# generate correlation matrix chart for numeric variables - with labels
# ggcorrplot(

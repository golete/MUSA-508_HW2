...
# join tracts with home data
eraWithTracts <- st_join(eraRecode, tracts) %>%
  rename(tractID = GEOID)

# add "neighborhoods"
eraTractsNhoods <- left_join(eraWithTracts, neighborhoodData, by = "MUSA_ID")

# create new data frame
etnSpatialLag <- eraTractsNhoods

# calculate spatial lag
coords <- st_coordinates(eraTractsNhoods)

neighborList <- knn2nb(knearneigh(coords, 5))

spatialWeights <- nb2listw(neighborList, style = "W")

# etnSpatialLag$lagPrice <- lag.listw(spatialWeights, etnSpatialLag$price)

#etnSpatialLag$lagLogPrice <- lag.listw(spatialWeights, etnSpatialLag$logPrice)

etnSpatialLag$logLagPrice <- log(lag.listw(spatialWeights, etnSpatialLag$price))

ggplot() +
  geom_sf(data = tracts) +
  geom_sf(data = neighborhoods, color = "blue", fill = "transparent") +
  geom_sf(data = eraWithTracts, aes(fill = "GEOID"))
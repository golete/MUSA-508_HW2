...
eraTractCommunityBTemp <- st_join(eraTractCity, subcommunities) %>%
  mutate(
    boulderCity = if_else(city == "Boulder", 1, 0),
    community = if_else(
      is.na(SUBCOMMUNITY), as.character(city), as.character(SUBCOMMUNITY)
    )
  )

eraTractCommunityB <- eraTractCommunityTemp %>%
  dplyr::select(-city, -SUBCOMMUNITY)
